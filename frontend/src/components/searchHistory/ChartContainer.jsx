    ...
    const handlePdfDownload = () => {
      ...
      import("jspdf")
        ...
        .catch((error) => {
          ...
          toast.error(
            isEnglish ? "Error creating PDF" : "შეცდომა PDF ფაილის შექმნისას"
          );
        });
          ...
          {isIntersecting && (
            <Suspense
              fallback={
                <LoadingSpinner
                  message={
                    isEnglish ? "Loading chart..." : "ჩარტი იტვირთება..."
                  }
                />
              }
            >
                ...
                <ReactECharts
                  ...
                  option={{
                    ...
                    legend: {
                      ...
                      formatter: (name) => {
                        const dataItem = dateGroup.data.find(
                          (item) => item.name === name
                        );
                        if (!dataItem)
                          return wrapText(name, 10, dateGroup.data.length);

                        // Calculate percentage from the data
                        const totalValue = dateGroup.data.reduce(
