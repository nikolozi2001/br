...
import { Card, HeroGradient, RoundButton, SectionLabel } from '../components/primitives';
import { cachedLookup } from '../api/lookupCache';
import { getStrings } from '../i18n/strings';
import { POLICY_KA_EDITION, POLICY_KA_SOURCE, policyKa } from '../data/policyKa';
import { policyApp } from '../data/policyApp';
import { parsePolicy, type PolicyBlock } from '../utils/policyText';
import { useTheme } from '../theme/ThemeProvider';
import type { SettingsScreenProps } from '../navigation/types';

/**
 * Two policies, in the order a reader needs them.
 *
 * First the app's own — what *this application* does with your data — which is
 * what the stores require and what someone tapping "Privacy policy" is asking
 * about. It used to exist only in the store listing, so the screen answered a
 * question nobody had asked. It is short and ships with the build.
 *
 * Then Geostat's, on how the authority treats respondents' data. It is the
 * longer, weightier document, but it is not about the reader's phone.
 *
 * Both are laid out by the app rather than shown in a WebView: the wording is
 * theirs, the presentation ours, so it honours the theme and the font-size
 * setting like every other screen. Only Geostat's English text is fetched — an
 * embedded copy would go stale the day they revise it — and it is cached
 * alongside the picker lists, so a second visit and an offline one both work.
 * Georgian exists only as a PDF, which the app cannot lay out, so it ships too;
 * see {@link policyKa}.
 */
...
export default function PrivacyScreen({ navigation }: SettingsScreenProps<'Privacy'>) {
  ...
  const renderBlock = (block: PolicyBlock, index: number) => {
    if (block.kind === 'heading') {
      return (
        <Text key={index} style={{ fontFamily: fonts.heading, fontSize: fs(17), color: colors.brand }}>
          {block.text}
        </Text>
      );
    }
  ...
  return (
    <View style={{ flex: 1, backgroundColor: colors.bg }}>
      <HeroGradient>
        ...
        >
          ...
          <View style={{ flex: 1 }}>
            ...
            <Text style={{ fontSize: fs(12), color: 'rgba(255,255,255,0.82)' }} numberOfLines={1}>
              {t.appName}
            </Text>
          </View>
        </View>
      ...
      <ScrollView contentContainerStyle={{ padding: 14, paddingBottom: 32 + insets.bottom, gap: 10 }}>
        <SectionLabel>{t.privacyThisApp}</SectionLabel>
        <Card style={{ padding: 16, gap: 12 }}>
          {app.blocks.map(renderBlock)}
          {/* The byline the published page carries under its title. */}
        ...
        <SectionLabel style={{ marginTop: 10 }}>{t.privacyGeostat}</SectionLabel>
        {/* Only this half can be missing: the English text comes off their site. */}
        {failed ? (
          <Card style={{ padding: 16 }}>
            <Text style={body}>{t.privacyOffline}</Text>
          </Card>
        ) : blocks ? (
          <>
            <Card style={{ padding: 16, gap: 12 }}>{blocks.map(renderBlock)}</Card>
            ...
            >
              <Text style={{ fontSize: fs(12), color: colors.brand }}>{t.privacySource(source.label)}</Text>
