...
**git:** `main` `origin/main`-ს **9 commit-ით უსწრებს** — `git push origin main` გასაკეთებელია.
...
```
a41565d    feat(settings): answer what the app collects before what the office does
d0c45fc    feat(settings): the confidentiality policy in Georgian
...
**ეკრანზე ორი დოკუმენტია, ამ თანმიმდევრობით:**

1. **აპის პოლიტიკა** — რას აგროვებს *აპლიკაცია* (არაფერს), საიდან მოდის რუკები, სად ინახება პარამეტრები. სწორედ ამას ეძებს ის, ვინც „კონფიდენციალურობის პოლიტიკას" აჭერს, და სწორედ ამას ითხოვენ მაღაზიები. მოკლეა, ბილდშია, ქსელი არ სჭირდება.
2. **საქსტატის პოლიტიკა** — რას აკეთებს უწყება რესპონდენტების მონაცემებთან. გრძელია და მნიშვნელოვანი, მაგრამ მკითხველის ტელეფონზე არაფერს ამბობს.

აქამდე მხოლოდ მეორე ჩანდა, ანუ ეკრანი სხვა კითხვას პასუხობდა.

**საქსტატის ტექსტის ორი ენა ორი გზით მოდის, რადგან ისინი მათ სხვადასხვანაირად აქვეყნებენ:**
...
აპის პოლიტიკაც ორ ასლადაა — `privacy-policy.html` (ლისტინგისთვის) და `policyApp.ts` (აპისთვის). აქ დავიწყებას ტესტი იჭერს: `policyApp.test.ts` HTML-ს კითხულობს და სხვაობაზე ვარდება.
...
**აუდიტი დასრულებულია (7 აგვისტო).** `src/`-ში 12 ფიქსირებული `lineHeight` იყო და **ყველა** მრავალსტრიქონიან ქართულ ტექსტზე იჯდა, თანაფარდობით 1.23–1.50 — ანუ ყველა იმაზე მჭიდრო, ვიდრე 1.53, რომელიც გატყდა. არცერთს `numberOfLines={1}` არ ჰქონდა. ყველა მოხსნილია.

ყველაზე მძიმე იყო არა EmptyState, არამედ სახელები: `DetailScreen.tsx:475` (hero, 1.23), `SubjectShareSheet.tsx:133` (1.26), `SubjectCard.tsx:47` (1.30). ბარათზე მოკლული დასახელება — „შპს ალფა ბეტა" ნაცვლად „შპს ალფა ბეტა გამა"-ს — სანდოდ გამოიყურება და მკითხველი ვერ მიხვდება, რომ ცდება.

დაბრუნებას [lineHeight.test.ts](mobile/src/theme/lineHeight.test.ts) იჭერს: `src/`-ს სკანერავს და ვარდება, თუ ფიქსირებული მნიშვნელობა გამოჩნდა (კომენტარებს არ ითვლის, ამიტომ ეს განყოფილება და `PrivacyScreen`-ის კომენტარი ტესტს არ არღვევს).

❗ **ვიზუალურად შეუმოწმებელია** — `mobile/node_modules` არ იყო დაინსტალირებული, ანუ `npm test`/`typecheck` არ გაშვებულა (ტესტის ლოგიკა ცალკე, Node-ით გადამოწმდა: 6/6). `DetailScreen.tsx:475`-ს ყველაზე მეტად სჭირდება თვალით ნახვა — მას `letterSpacing: -0.33`-იც აქვს, ანუ შეგნებულად შემჭიდროებული სათაურია და `lineHeight`-ის მოხსნა მას ოდნავ გაათხელებს.
...
- **აპის პოლიტიკა ორ ადგილას წერია და ერთმანეთს უნდა ემთხვეოდეს.** [privacy-policy.html](mobile/privacy-policy.html) მაღაზიების ლისტინგისთვისაა, [policyApp.ts](mobile/src/data/policyApp.ts) კი აპისთვის — ერთი დოკუმენტის ორი ასლი. **ერთის შეცვლისას მეორეც უნდა შეიცვალოს**; `policyApp.test.ts` HTML-ს კითხულობს და სხვაობაზე ვარდება, ანუ დავიწყება ტესტში გამოჩნდება.
- **აპის პოლიტიკაში ექსპორტი არ არის ნახსენები.** 4.3.0-დან აპი ფაილებს მოწყობილობაზე ინახავს, iOS-ზე გრაფიკი **ფოტოებში** ხვდება — პოლიტიკა ამაზე არაფერს ამბობს. სამართლებრივი ტექსტია, ამიტომ ჩემით არ დამიმატებია; შესწორება ორივე ასლში ერთდროულად უნდა წავიდეს.
- **ბავშვების პუნქტი ამოღებულია** (6 აგვისტო, მფლობელის გადაწყვეტილებით). ადრე ეწერა, რომ აპი 13 წლამდე ბავშვებზე არ არის მიმართული. თუ მაღაზიები ოდესმე ასაკობრივ დეკლარაციას მოითხოვენ, ეს წინადადება დასაბრუნებელია — **ორივე ასლში**, ნუმერაციის გასწორებით.
...
6 აგვისტოს: კონფიდენციალურობის ეკრანი ორივე ენაზე, თავიდან ბოლომდე — ორივე დოკუმენტი, ქართული საქსტატის ტექსტი (სათაური, ა)/ბ) ქვეპუნქტები, 10 პრინციპი, წყაროს ბმული), ინგლისურის ჩამოტვირთვა, სამივე ფონტის ზომა.
...
- `npm test` → **116 ტესტი**, `npm run typecheck` → სუფთა
...
7. **`lineHeight`-ის ვიზუალური გადამოწმება** — გრძელდასახელებიანი სუბიექტი სამივე ფონტის ზომაზე (კოდში აუდიტი დასრულებულია, იხ. „გაკვეთილი" ზემოთ)
